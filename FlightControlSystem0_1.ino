...
int PWMMin = 61;   // minimum PWM value that causes the motors to spin
...
void loop() {
 ...
 if (ESC_ON) {
    ESC_RT.write(PWMValue);
    ESC_LT.write(PWMValue);
    ESC_TT.write(PWMValue);
    ESC_RB.write(PWMValue);
    ESC_LB.write(PWMValue);
    ESC_TB.write(PWMValue);
    delay(100);
  }
  else {
    ESC_RT.write(PWMZero);
